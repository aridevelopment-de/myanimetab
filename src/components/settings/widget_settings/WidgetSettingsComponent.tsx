...
const containsString = (string: string, component: Component) => {
    ...
    if (component.contentSettings !== undefined) {
        ...
        for (let i = 0; i < component.contentSettings.length; i++) {
            if (component.contentSettings[i].name.toLowerCase().includes(string)) {
                return true;
            }
        }
    }
    ...
    return (
        <React.Fragment>
            ...
            <div className={styles.control_group}>
                <button className={styles.import_button} onClick={() => EventHandler.emit("import_window_state", { opened: true })}>Import</button>
                <button className={styles.export_button} onClick={() => EventHandler.emit("export_window_state", { opened: true })}>Export</button>
            </div>
