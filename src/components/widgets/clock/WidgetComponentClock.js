...
function Clock(props) {
    const [ position, _ ] = useSetting("cc.clock.position", "clock");
    const [ timeFormat, _1 ] = useSetting("cc.clock.time_format", "clock");
    const [ opacity, setOpacity ] = useState(0);
    const [ currentTime, setCurrentTime ] = useState(TimeUtils.convertTimeToClockFormat(new Date(), timeFormat === 1));

    useEvent("blurall", "clock", 1, (data) => setOpacity(data.blur ? getUserSettings().get("cc.clock.auto_hide") : 0));
    useEffect(() => {
