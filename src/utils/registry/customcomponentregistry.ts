...
class CustomComponentRegistry {
    ...
    loadKnownComponents() {
		...
		components.forEach((data: { name: string, entryPoint: string }) => {
			...
			const metadata: KnownComponent = require(`../../components/widgets/${data.entryPoint}.tsx`).default;
    ...
    _prepareSettings(component: KnownComponent) {
        ...
        if (component.headerSettingd?.option.type !== null) {
            settings.state = component.headerSettingd?.option.default;
        }
    ...
    async setupDefaultComponents() {
        await asyncFilter(this.knownComponents, async(component: KnownComponent) => {
            if (component.metadata.defaultComponent === true) {
                const widget = await widgetsDb.getWidget(`${component.type}-${0}`);

                if (widget === undefined) {
                    await widgetsDb.addWidget(component.type, this._prepareSettings(component));
                }
            }
        });
    }
    ...
    loadInstalledComponents(callback) {
        ...
        widgetsDb.getIdentifiers().then(identifiers => {
            identifiers.forEach(identifier => {
                const component: KnownComponent = this.knownComponents.find(c => c.type === identifier.id);

                if (component === undefined) {
                    throw new Error(`Component type ${identifier.id} is not known`);
                }                
