...
class WidgetDatabase extends Dexie {
    ...
    async registerWidget(type: string, settings?: { [key: string]: any }) {
    ...
    async addWidget(type: string, settings?: { [key: string]: any }) {
        let amt = 0;
        this.getIdentifiers().then(identifiers => amt = identifiers.filter(identifier => identifier.id === type).length);
    ...
    async setSetting(id: string, key: string, value: any) {
...
class MetaDatabase extends Dexie {
    ...
    async getMeta(name: string): Promise<any | undefined> {
        return this.meta.get(name).then(meta => (meta || { value: undefined }).value);
    }

    async setMeta(name: string, value: any) {
        this.meta.update(name, { value: value });
        this.emitOnMetaChange(name, value);
    }

    async registerMeta(name: string, value: any) {
